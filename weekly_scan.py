#!/usr/bin/env python3
"""Weekly Inside CPR Scanner — runs Friday evening after market close.
Uses weekly OHLC (Mon-Fri) to find stocks where NEXT week's CPR is
completely inside THIS week's CPR (weekly version of the daily scan.py).
Writes docs/weekly_cpr_list.json.
"""
...
def send_email_photo(image_bytes, subject, body, filename="weekly_inside_cpr.png"):
    if not (GMAIL_ADDRESS and GMAIL_APP_PASSWORD and REPORT_RECIPIENT):
        print("[email] missing credentials")
        return False
    try:
...
def make_poster(stocks, target_label, header_title="WEEKLY INSIDE CPR STOCKS"):
    n = len(stocks)
    use_two_col = n > 5
    ...
    center_text(d, header_title, font(58), W / 2, y_title_top + 35, WHITE)
...
def export_cpr_list_json(stocks_data, target_label, path="docs/weekly_cpr_list.json"):
        ...
        "stocks": [
            {
                ...
                "r1":        round(s["pv"]["r1"], 2),
                "s1":        round(s["pv"]["s1"], 2),
                "week_high": round(s["wk_high"], 2),
                "week_low":  round(s["wk_low"], 2),
            }
...
def next_week_label(last_friday_date):
    """Given the Friday just closed, label the upcoming trading week Mon-Fri,
    nudging the start date forward past weekends/holidays if needed."""
    start = last_friday_date + datetime.timedelta(days=3)  # -> Monday
    while start.weekday() >= 5 or start in NSE_HOLIDAYS:
...
def main():
    ...
    print(f"Downloading {len(tickers)} tickers (90d history for weekly resample)...")
    try:
        df = yf.download(tickers, period="90d", group_by='ticker',
                         auto_adjust=False, progress=False, threads=True, timeout=90)
    except Exception as e:
        send_email_text("Weekly CPR Scanner Error", f"yfinance error: {e}")
        sys.exit(1)
    if df is None or df.empty:
        send_email_text("Weekly CPR Scanner Error", "yfinance returned empty data")
        sys.exit(1)

    print("Processing weekly resample...")
    inside = []
    week_label = None
    ...
    for sym in STOCKS:
        ...
        try:
            ...
            # weekly.iloc[-1] = the week that just closed today (Friday)
            # weekly.iloc[-2] = the week before that
            just_closed = weekly.iloc[-1]
            prior_week  = weekly.iloc[-2]

            if week_label is None:
                last_friday = weekly.index[-1].date()
                week_label = next_week_label(last_friday)
                print(f"Target week: {week_label}")

            this_week_cpr = calc_cpr(float(prior_week['High']), float(prior_week['Low']), float(prior_week['Close']))
            next_week_cpr = calc_cpr(float(just_closed['High']), float(just_closed['Low']), float(just_closed['Close']))

            if (next_week_cpr['upper'] <= this_week_cpr['upper'] and
                next_week_cpr['lower'] >= this_week_cpr['lower']):
                w_pct = (next_week_cpr['width'] / float(just_closed['Close'])) * 100
                ...
                inside.append({
                    ...
                    'pv': next_week_cpr,
                    'wk_high': float(just_closed['High']),
                    'wk_low': float(just_closed['Low']),
                })
        except Exception:
            continue

    inside.sort(key=lambda x: x['w_pct'])
    print(f"Found {len(inside)} weekly inside CPR stocks")

    export_cpr_list_json(inside, week_label or "")

    if not inside:
        send_email_text(
            f"Weekly Inside CPR Stock List — {week_label}",
            f"<b>Weekly Inside CPR Stock List</b><br>For week: {week_label}<br><br>No weekly inside CPR stocks."
        )
        return
    ...
    img_buf = make_poster(syms, week_label)
    subject = f"📊 Weekly Inside CPR Stock List — {week_label}"
    body = (
        f"<h2>Weekly Inside CPR Stock List</h2>"
        f"<p>For the trading week: <b>{week_label}</b></p>"
